...
/*******************************************************************************
    ...
    This is a branch of CRUD (taken from the 1.1 version) that uses PHP's 
    mysqli interface instead of PDO. Created for demonstration purposes only. 
    This version will not be maintained. 

*******************************************************************************/

define("VERSION", "1.2 mysqli");
...
function list_albums()
{
    ...
    $qrh = get_albums_sql();    // query handle from SQL
...
// perform the query and return qrh (query result handle)
...
function insert_track_sql ( $track )
{
    ...
    $sth = $dbh->prepare($query);
    if($sth) {
        $sth->bind_param("ssss", @$track['album_id'], @$track['track_number'], @$track['title'], @$track['duration'] );
        $sth->execute();
    } else {
        error("insert_track_sql: insert prepare returned no statement handle");
    }
...
function update_album_sql( $album )
{
    ...
    $sth = $dbh->prepare($query);
    if($sth) {
        $sth->bind_param("ssssi", @$album['title'], @$album['artist'], @$album['label'], @$album['released'], @$album['id'] );
        $sth->execute();
    } else {
        error("update_album_sql: update prepare returned no statement handle");
    }
...
function update_track_sql ( $track )
{
    ...
    $sth = $dbh->prepare($query);
    if($sth) {
        $sth->bind_param("sssi", @$track['track_number'], @$track['title'], @$track['duration'], @$track['id'] );
        $sth->execute();
    } else {
        error("update_track_sql: update prepare returned no statement handle");
    }
...
function delete_album_sql( $id )
{
    ...
    $query1 =  "DELETE FROM track  WHERE album_id = $id";
    $query2 =  "DELETE FROM album  WHERE id = $id";

    // delete tracks
    $qrh = $dbh->query($query1);
...
function _init( )
{
    ...
    foreach ( array(
            'BUTTONS', 'HIDDENS', 'MESSAGES', 'ERRORS', 'CONTENT', 'PRECONTENT', 'POSTCONTENT',
    ...
    $hostname = 'localhost';  // mysqli doesn't support persistent connections with the default mysql driver

    $dbh = @new mysqli($hostname, MYSQLUSER, MYSQLPASS, MYSQLDB )
        or error('cannot initialize mysqli');

    // this is a bug workaround for pre-5.3 PHP
    // should use $dbh->connect_error
